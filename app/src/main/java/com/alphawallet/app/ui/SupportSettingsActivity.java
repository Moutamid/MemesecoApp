...
//        supportSettingsLayout.addView(faq);
    ...
    private void onBlogClicked() {
        Intent intent = new Intent(Intent.ACTION_VIEW);
        intent.setData(Uri.parse(MediaLinks.AWALLET_BLOG_URL));
        if (isAppAvailable(C.MEDIUM_PACKAGE_NAME)) {
            intent.setPackage(C.MEDIUM_PACKAGE_NAME);
        }
        try {
            startActivity(intent);
        } catch (Exception e) {
